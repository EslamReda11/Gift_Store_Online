...
@section('content')
    <div align="left" class="px-5 py-2" style="background:#1CD5E8;margin-top:5px; ">
        ...
        <p class="my-2 " style="font-weight:bold; "><a href="{{ url('/') }}" class="black-text">Home </a>/
            {{ $Product->name }} </p>
    ...
    <script>
        $(document).ready(function() {
            $("#main_image, .xzoom-gallery").xzoom({
                fadeIn: true;fadeOut: true;position: inside;
            });
        ...
        <div class="product_data px-3 py-5  ">
            ...
            <div class="row  d-flex align-items-center ">
                <div class="col-md-4  animated fadeInRight fast" style="width: 100%;">
                    ...
                    <img class="xzoom img-fluid" id="main_image" src="{{ asset('Uploads/Products/' . $Product->image1) }}"
                        xoriginal="{{ asset('Uploads/Products/' . $Product->image1) }}" style="width:70%">
                    <!-- Thumbnails -->
                    <br> <br>
                    ...
                    @else
                        <a href="{{ asset('Uploads/Products/' . $Product->image2) }}">
                            <img class="xzoom-gallery" width="80"
                                ...
                                xpreview="{{ asset('Uploads/Products/' . $Product->image2) }}">
                        </a>
                    @endif
                    ...
                    @else
                        <a href="{{ asset('Uploads/Products/' . $Product->image3) }}">
                            <img class="xzoom-gallery" width="80"
                                src="{{ asset('Uploads/Products/' . $Product->image3) }}">
                        </a>
                    @endif
                    @if ($Product->image3 == '')
                    @else
                        <a href="{{ asset('Uploads/Products/' . $Product->image4) }}">
                            <img class="xzoom-gallery" width="80"
                                src="{{ asset('Uploads/Products/' . $Product->image4) }}">
                        </a>
                    @endif
                </div>
                ...
                <div id="product_data" class="col-md-7 animated fadeInLeft fast">
                    ...
                    <input type="hidden" name="product_id" min=0 value="{{ $Product->id }}" required
                        class="form-control product_id">
                    <p> Price : <strong style="font-size:20px;font-family: 'Balsamiq Sans', cursive;">EGP
                            {{ $Product->price }}</strong></p>
                    <?php echo $Product->additional_info; ?>
                    <div class="col-md-6" style="margin-left:-20px;">
                        <input type="number" class="form-control quantity" value="1" name="quantity" placeholder="Quantity">
                    </div>
