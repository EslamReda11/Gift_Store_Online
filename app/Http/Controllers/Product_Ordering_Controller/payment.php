...
Your Order is Confirmed. Estimated Delivery 3-5 Working days</p>
            ...
            Mail::send(array('html' => 'emails.order_email'), $emailcontent, function ($message) use ($loginid, $name, $O_id) {
                $message->to($loginid, $name)->subject('Your db_store.com Order Payment ' . $O_id . ' is Successfully Paid');
                $message->from('codetalentum@btao.in', 'db_store');
            });
            ...
            $emailbody = ' <p>Your Payment ' . $amount . ' towards Order ' . $O_id . 'is failed. <br>
        	                ...
        	                <a href="https://www.db_store.com/proceed_to_Payment/' . $O_id . '">https://www.db_store.com/proceed_to_Payment/' . $O_id . '</a></p>
        	                <h4>Order Details: </h4><p> Order No:' . $O_id . $Order_Details . '</p>
        	                 <p><strong>Delivery Address:</strong>
        	               ' . $Delivery_Address . '</p>
        	                <p> <strong>Total Amount:</strong>
            ...
            Mail::send(array('html' => 'emails.order_email'), $emailcontent, function ($message) use ($loginid, $name, $O_id) {
                $message->to($loginid, $name)->subject('Your db_store Order ' . $O_id . ' Payment is Failed');
                $message->from('codetalentum@btao.in', 'db_store');
            });
            Session::forget('cart');
            Session::forget('discount');
